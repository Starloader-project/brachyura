...
public class PluginMetadata {
    ...
    public static PluginMetadata read(BufferedReader r) {
        try {
            ...
            // Slbrachyura: Added null checks
            if (header == null) {
                throw new EOFException("Plugin meta header not present");
            }
            String mvnid = r.readLine();
            if (mvnid == null) {
                throw new EOFException("Plugin maven id not present");
            }
            PluginMetadata result = new PluginMetadata(new MavenId(mvnid));
            String depId;
